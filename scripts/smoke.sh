...
# Cabecera para los endpoints que MUTAN estado. Si el backend tiene
# HUB_ADMIN_TOKEN puesto (o sea, esta expuesto), sin esto responden 401 y medio
# smoke falla sin que haya nada roto. Vacia si no hay token: no estorba.
# Sin espacios NI comillas a proposito: curl acepta -HCabecera:valor pegado, y
# asi la variable sobrevive al word-splitting de bash. Con `-H "X-Hub-Token: x"`
# bash la parte en tres pedazos, la cabecera sale rota y todo responde 401.
TOKARG=""
[ -n "${HUB_ADMIN_TOKEN:-}" ] && TOKARG="-HX-Hub-Token:$HUB_ADMIN_TOKEN"
ok=0; fallo=0
...
echo "-- integridad contra el sandbox real (export 4034 del 12/09)"
check "las 15 geocercas reales sembradas" \
  "curl -s $B/api/ps/geocercas | python3 -c \"import json,sys;print(len(json.load(sys.stdin)))\"" '15'
check "PROY-001 trae su coordenada real" \
  "curl -s $B/api/ps/geocercas" '13.9290675'
check "PROY-014 confirma el escalado 1e7" \
  "curl -s $B/api/ps/geocercas" '13.3376152'
check "ninguna maquina fantasma sembrada" \
  "curl -s $B/api/ps/maquinas | grep -c 'EXC22017LC\\|RC03004EC\\|RM19003EC' || true" '0'
check "EXC-02 existe en el sandbox" "curl -s $B/api/ps/maquinas" 'EXC-02'
check "EXC-01 sigue en 1200" "curl -s $B/api/ps/maquinas" '1200'
...
curl -s --max-time 5 -X POST $TOKARG "$B/admin/base-diesel?asset=CAT-320-04&consumido=60" >/dev/null 2>&1
check "un vale de 120 gal contra 60 consumidos abre ticket" \
  ...
  'Descuadre de diesel'
curl -s --max-time 5 -X POST $TOKARG "$B/admin/base-diesel?asset=$EXCAV&consumido=60" >/dev/null 2>&1
...
check "pasa a EN_MANTENIMIENTO y el taller lo puede cerrar" \
  "curl -s -X POST $TOKARG '$B/admin/vencer-servicio?asset=$EXCAV' >/dev/null; j /ingest/telemetry/cat '{\"EquipmentHeader\":{\"SerialNumber\":\"$EXCAV\"},\"SnapshotTime\":\"2026-09-12T14:35:00Z\",\"EngineStatus\":{\"Running\":true}}' | grep -o EN_MANTENIMIENTO; curl -s -X POST $B/api/assets/$EXCAV/servicio" \
  'servicio cerrado'
...
check "el estado de tarea se decide por workflow_role, no por el nombre" \
  "$PS/vista-unificada/RE-01 | python3 -c \"import json,sys; print(json.load(sys.stdin)[0]['tarea']['workflow_role'])\"" \
  '^0$'
...
check "mantenimiento reportado por los DOS sistemas a la vez" \
  "$PS/vista-unificada/RE-01" 'Las DOS plataformas lo saben'
check "y Startrack lo tiene en su propio catalogo de estado de vehiculo" \
  "$PS/vista-unificada/RE-01 | python3 -c \"import json,sys; print(json.load(sys.stdin)[0]['tarea']['estado_vehiculo'])\"" \
  '^1$'
...
check "con medicion real, la exposicion se descuenta" \
  "exposicion_de EXC-02 horas_medidas" '^7.5$'
check "y el equipo conectado casi no expone nada: la KPI no grita lobo" \
  "exposicion_de EXC-02 horas_sin_respaldo" '^0.5$'
...
check "sin credenciales el Hub lo dice y no se rompe" \
  "curl -s -X POST $TOKARG $B/admin/sync-startrack" 'sin credenciales'
...
check "sin credenciales de Prisma tampoco se rompe" \
  "curl -s -X POST $TOKARG $B/admin/sync-prisma" 'sin credenciales'
...
check "acepta el formato 2 (lote) que tambien documentan" \
  "curl -s -X POST $WH/ubicaciones -H 'content-type: application/json' -d '[1,[{\"code\":0,\"vid\":939,\"remote_id\":\"EXC-17007EC\",\"hourmeter\":1482.0,\"event_time\":\"2026-09-13T08:00:00-06:00\"}]]'" \
  '"procesados": *1'
...
check "las coordenadas escaladas del formato 2 se normalizan a grados" \
  "curl -s -X POST $WH/ubicaciones -H 'content-type: application/json' -d '[1,[{\"code\":0,\"vid\":939,\"remote_id\":\"EXC-17007EC\",\"lat\":133376152,\"lon\":-878486967,\"event_time\":\"2026-09-13T09:00:00-06:00\"}]]' >/dev/null; $PS/vista-unificada/EXC-02 | python3 -c \"import json,sys; t=json.load(sys.stdin)[0]['tarea']; print(round(t['lat'],4), round(t['lon'],4))\"" \
  '^13.3376 -87.8487$'
check "y las decimales del formato 1 pasan intactas" \
  "curl -s -X POST $WH/ubicaciones -H 'content-type: application/json' -d '{\"code\":0,\"vid\":939,\"remote_id\":\"EXC-17007EC\",\"lat\":\"13.92\",\"lon\":\"-89.84\",\"event_time\":\"2026-09-13T09:05:00-06:00\"}' >/dev/null; $PS/vista-unificada/EXC-02 | python3 -c \"import json,sys; t=json.load(sys.stdin)[0]['tarea']; print(t['lat'], t['lon'])\"" \
  '^13.92 -89.84$'
...
curl -s -X POST $TOKARG "$B/admin/seed-ps" >/dev/null
PEA=$(curl -s -X POST "$B/api/ps/pea/EXC-02?aprobado_por=prueba-smoke")
...
check "y queda en el log append-only" \
  "curl -s $PS/pea" '"maquinaria": *"EXC-02"'
...
curl -s -X POST $TOKARG "$SIN/limpiar" >/dev/null
...
check "genera el inventario pedido" \
  "curl -s -X POST $TOKARG '$SIN/generar?equipos=40&seed=42'" '"equipos": *40'

H_A=$(huella)
curl -s -X POST $TOKARG "$SIN/limpiar" >/dev/null
curl -s -X POST $TOKARG "$SIN/generar?equipos=40&seed=42" >/dev/null
H_B=$(huella)
curl -s -X POST $TOKARG "$SIN/limpiar" >/dev/null
curl -s -X POST $TOKARG "$SIN/generar?equipos=40&seed=99" >/dev/null
H_C=$(huella)
