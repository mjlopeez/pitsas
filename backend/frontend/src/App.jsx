...
export default function App() {
  ...
  const mandarCampo = useCallback(async (payload) => {
    try {
      const r = await api.mandarMensajeCampo(payload)
      setSimRes(r)
      setSel(r.resultado?.asset_identifier || null)
    } catch (e) { alert(`El mensaje no se pudo procesar: ${e.message}`) }
  }, [])
