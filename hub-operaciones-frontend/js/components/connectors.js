...
function renderConnectors(container) {
  ...
  container.innerHTML = `
    <div class="space-y-6">
      ...
      <div class="gl-card p-6 sm:p-8 bg-white border border-[#e8e6e1]">
        <span class="gl-subtitle text-[#bf9410] block mb-1">ARQUITECTURA DE INGESTA CANÓNICA • ISO 15143-3</span>
      ...
      <div class="gl-card p-5 sm:p-6 bg-white border border-[#e8e6e1]">
        <div class="flex items-center justify-between pb-3 mb-3 border-b border-[#e8e6e1]">
          <div>
            <span class="gl-subtitle text-[#bf9410] block mb-0.5">CANALES DE INGESTA</span>
            <h3 class="font-editorial-serif text-lg font-normal text-[#111111]">Protocolos Técnicos y Fabricantes Homologados</h3>
          </div>
        ...
        <div class="gl-table-wrap">
          <table class="gl-table">
            ...
            <tbody>
              ${connectors.length === 0 ? `
                <tr><td colspan="6" class="py-6 text-center text-[#777777]">Cargando conectores telemáticos...</td></tr>
              ` : connectors.map(c => `
                <tr>
                  <td class="font-bold text-[#111111] flex items-center gap-2">
                    <span class="w-2 h-2 rounded-full ${c.estado === 'activo' ? 'bg-emerald-500' : 'bg-slate-400'}"></span>
                    ${c.nombre}
                  </td>
                  <td class="font-mono text-[#0e2439]">${c.protocolo}</td>
                  <td>
                    <span class="gl-badge ${c.estado === 'activo' ? 'gl-badge-success' : 'gl-badge-gold'} text-[9px]">
                      ${c.estado.replace('_', ' ')}
                    </span>
                  </td>
                  <td class="font-mono text-[#15803d] font-bold">${c.aceptados || 0}</td>
                  <td class="font-mono text-[#c62828] font-bold">${c.rechazados || 0}</td>
                  <td class="font-mono text-[#777777] text-[11px]">${c.ultimo_evento ? new Date(c.ultimo_evento).toLocaleTimeString() : 'En espera'}</td>
                </tr>
              `).join('')}
            </tbody>
          </table>
      ...
      <div class="gl-card p-6 sm:p-8 bg-white border border-[#e8e6e1]">
        <div class="flex items-center justify-between pb-3 mb-4 border-b border-[#e8e6e1]">
          <div>
            <span class="gl-subtitle text-[#bf9410] block mb-0.5">ENTORNO DE PRUEBAS DE CARGA</span>
            <h3 class="font-editorial-serif text-lg font-normal text-[#111111]">Generador de Escenarios & Pruebas Sintéticas</h3>
          ...
          <i data-lucide="cpu" class="w-6 h-6 text-[#bf9410]"></i>
        </div>

        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
          
          <div class="p-4 rounded border border-[#e8e6e1] bg-[#faf9f6] flex flex-col justify-between">
            <div>
              <strong class="text-[#111111] block mb-1">Generar Dataset Sintético</strong>
              <p class="text-[#666666] text-[11px] mb-3">Inyecta 120 equipos respetando proporciones reales para pruebas.</p>
              <div class="flex items-center gap-2 mb-3">
                <span class="text-[#777777]">Seed:</span>
                <input type="number" id="synthetic-seed" value="42" class="w-20 px-2 py-1 bg-white border border-[#d6d3cb] rounded text-center text-[#111111] font-mono">
                <span class="text-[#777777] ml-2">Equipos:</span>
                <input type="number" id="synthetic-count" value="120" class="w-20 px-2 py-1 bg-white border border-[#d6d3cb] rounded text-center text-[#111111] font-mono">
              </div>
            </div>
          ...
          <div class="p-4 rounded border border-[#e8e6e1] bg-[#faf9f6] flex flex-col justify-between">
            <div>
              <strong class="text-[#111111] block mb-1">Limpieza de Datos Sintéticos</strong>
              <p class="text-[#666666] text-[11px] mb-3">Elimina únicamente los datos generados de prueba, conservando la flota real de la demo intacta.</p>
            </div>
            <button id="btn-clean-synthetic" class="gl-btn-outline w-full justify-center text-[#c62828] border-[#fca5a5] hover:bg-rose-50">
              <i data-lucide="trash-2" class="w-4 h-4 text-[#c62828]"></i>
  ...
  if (btnGen) {
    btnGen.addEventListener('click', async () => {
      ...
      btnGen.innerHTML = `<i data-lucide="loader-2" class="w-4 h-4 animate-spin text-[#bf9410]"></i><span>Generando...</span>`;
      ...
      try {
        ...
        btnGen.innerHTML = `<i data-lucide="play" class="w-4 h-4 text-[#bf9410]"></i><span>Generar Inventario</span>`;
        if (window.lucide) window.lucide.createIcons({ root: btnGen });
      }
    });
  }
  ...
  if (btnClean) {
    btnClean.addEventListener('click', async () => {
      ...
      btnClean.innerHTML = `<i data-lucide="loader-2" class="w-4 h-4 animate-spin text-[#c62828]"></i><span>Limpiando...</span>`;
      ...
      try {
        ...
        btnClean.innerHTML = `<i data-lucide="trash-2" class="w-4 h-4 text-[#c62828]"></i><span>Limpiar Solo Sintéticos</span>`;
        if (window.lucide) window.lucide.createIcons({ root: btnClean });
      }
    });
  }
