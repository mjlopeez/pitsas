  ...
  container.innerHTML = `
    <div class="space-y-6">
      ...
      <div class="gl-card p-6 sm:p-8 bg-white border border-[#e8e6e1]">
        <span class="gl-subtitle text-[#bf9410] block mb-1">REGULACIÓN VIAL & LOGÍSTICA • EL SALVADOR</span>
        ...
        <p class="text-xs sm:text-sm text-[#555555] max-w-3xl leading-relaxed">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ${corridors.map(cor => {
          ...
          return `
            <div class="gl-card p-5 sm:p-6 bg-white border border-[#e8e6e1] flex flex-col justify-between">
              <div>
                <div class="flex items-start justify-between gap-2 border-b border-[#e8e6e1] pb-3 mb-3">
                  <div>
                    <h4 class="font-bold text-[#111111] text-sm">${cor.nombre}</h4>
                    <span class="font-mono text-[11px] text-[#0e2439]">ID: ${cor.corredor_id}</span>
                  </div>
                ...
                <div class="space-y-2 text-xs mb-4">
                  <div class="flex justify-between text-[#555555]">
                    <span class="text-[#777777]">Penalidad de Tiempo:</span>
                    <span class="font-mono text-[#111111] font-bold">+${cor.penalidad_min} minutos</span>
                  </div>
                  <div class="flex justify-between text-[#555555]">
                    <span class="text-[#777777]">Aplicación:</span>
                    <strong class="text-[#111111]">Lunes a Viernes</strong>
                  </div>
                </div>

                <!-- Lista de Franjas de Veda -->
                <div class="space-y-1.5">
                  <span class="text-[9px] uppercase font-bold text-[#777777] tracking-wider">Horarios de Veda Vigentes:</span>
                  ${hasVedas ? cor.vedas.map(v => `
                    <div class="p-2.5 rounded border border-[#ecd99a] bg-[#fdfaf0] flex items-center justify-between text-xs font-mono">
                      <span class="text-[#8c6a05] font-bold">${v.desde} – ${v.hasta}</span>
                      <span class="text-[#555555] capitalize text-[11px] font-sans">${v.etiqueta}</span>
                    </div>
                  `).join('') : `
                    <div class="p-2.5 rounded border border-[#bbf7d0] bg-[#f0fdf4] text-[#15803d] text-xs font-semibold">
      ...
      <div class="gl-card p-6 sm:p-8 bg-white border border-[#e8e6e1]">
        <div class="flex items-center gap-2 mb-2">
          <i data-lucide="compass" class="w-5 h-5 text-[#bf9410]"></i>
          <h3 class="font-editorial-serif text-xl font-normal text-[#111111]">Validador de Salida de Carga (Algoritmo VMT)</h3>
        </div>
        <p class="text-xs text-[#555555] mb-4 max-w-2xl leading-relaxed">
        ...
        <form id="form-check-dispatch" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 text-xs mb-4">
          <div>
            <label class="block font-bold text-[#111111] mb-1">Tipo Material:</label>
            <select id="val-tipo" class="w-full px-3 py-2 bg-white border border-[#d6d3cb] rounded text-[#111111]">
          ...
          <div>
            ...
            <select id="val-origen" class="w-full px-3 py-2 bg-white border border-[#d6d3cb] rounded text-[#111111]">
          ...
          <div>
            ...
            <select id="val-destino" class="w-full px-3 py-2 bg-white border border-[#d6d3cb] rounded text-[#111111]">
          ...
          <div>
            <label class="block font-bold text-[#111111] mb-1">Corredor Propuesto:</label>
            <select id="val-corredor" class="w-full px-3 py-2 bg-white border border-[#d6d3cb] rounded text-[#111111]">
          ...
          <div class="sm:col-span-2 lg:col-span-4 flex justify-end pt-2">
            <button type="submit" class="gl-btn-black">
              <i data-lucide="check-circle" class="w-4 h-4 text-[#bf9410]"></i>
  ...
  if (formCheck && resultBox) {
    formCheck.addEventListener('submit', async (e) => {
      ...
      if (submitBtn) {
        ...
        submitBtn.innerHTML = `<i data-lucide="loader-2" class="w-4 h-4 animate-spin text-[#bf9410]"></i><span>Evaluando Veda VMT...</span>`;
        if (window.lucide) window.lucide.createIcons({ root: submitBtn });
      }

      try {
        resultBox.classList.remove('hidden');
        resultBox.innerHTML = `<span class="text-xs text-[#555555]">Consultando algoritmos de despacho del Hub...</span>`;
        ...
        resultBox.innerHTML = `
          ...
          <div class="space-y-1 mt-2 text-[#444444]">
            <div><strong class="text-[#111111]">Recomendación:</strong> ${res.mensaje || res.explicacion || 'Salida autorizada sin conflicto con vedas del VMT.'}</div>
            ${res.corredor_recomendado ? `<div><strong class="text-[#111111]">Corredor Óptimo:</strong> <span class="font-mono text-[#0e2439] font-bold">${res.corredor_recomendado}</span></div>` : ''}
          </div>
        `;
        ...
        if (submitBtn) {
          ...
          submitBtn.innerHTML = `<i data-lucide="check-circle" class="w-4 h-4 text-[#bf9410]"></i><span>Evaluar Ruta y Veda VMT</span>`;
          if (window.lucide) window.lucide.createIcons({ root: submitBtn });
        }
      }
    });
